...
const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const upcomingInterviews = await Interview.find({
    ...
    .populate('candidate', 'username email profile profilePicture profileImage')
    .populate('job', 'title')
    .sort({ scheduledAt: 1 })
    .limit(5);
  ...
  const recentApplications = await Application.find({ job: { $in: jobIds } })
    .populate('applicant', 'username email profile profilePicture profileImage')
    .populate('job', 'title')
    .sort({ createdAt: -1 })
    .limit(10);
...
const getATSBoard = asyncHandler(async (req, res) => {
  ...
  const applications = await Application.find({ job: { $in: jobIds } })
    .populate('applicant', 'username email profile phone profilePicture profileImage')
...
const addApplicationNote = asyncHandler(async (req, res) => {
  ...
  const populatedNote = await ApplicationNote.findById(note._id)
    .populate('author', 'username email profilePicture profileImage');
...
const scheduleInterview = asyncHandler(async (req, res) => {
  ...
  const populatedInterview = await Interview.findById(interview._id)
    .populate('candidate', 'username email phone profilePicture profileImage')
    .populate('job', 'title')
...
const getInterviews = asyncHandler(async (req, res) => {
  ...
  const interviews = await Interview.find(query)
    .populate('candidate', 'username email phone profile profilePicture profileImage')
    .populate('job', 'title')
    .populate('company', 'name logo')
    .sort({ scheduledAt: 1 });

...
const JobAlerts = () => {
  ...
  const fetchAlerts = async () => {
    try {
      setLoading(true);
      const { data } = await candidateApi.get('/job-alerts');
      setAlerts(data.data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingAlert) {
        await candidateApi.put(`/job-alerts/${editingAlert}`, alertData);
      } else {
        await candidateApi.post('/job-alerts', alertData);
      }
      fetchAlerts();
      resetForm();
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await candidateApi.delete(`/job-alerts/${id}`);
  ...
  const toggleAlert = async (id, isActive) => {
    try {
      await candidateApi.put(`/job-alerts/${id}`, { isActive: !isActive });
      fetchAlerts();
    } catch (error) {

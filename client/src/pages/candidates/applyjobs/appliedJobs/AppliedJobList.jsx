...
export default function AppliedJobs() {
  ...
  const stats = useMemo(() => {
    ...
    const pending = applications.filter((a) => ["Pending", "Reviewing"].includes(a.status)).length;
    const shortlisted = applications.filter((a) => a.status === "Shortlisted").length;
    const approved = applications.filter((a) => ["Selected", "Accepted"].includes(a.status)).length;
...
function ApplicationCard({ application, navigate }) {
  ...
  const formatSalary = (salary) => {
    ...
    const firstRange = salaryTypes
      .map((type) => salary[type])
      .find((range) => range?.min || range?.max);
    if (!firstRange) return "";
    const min = firstRange.min ? `₹${Number(firstRange.min).toLocaleString("en-IN")}` : "";
    const max = firstRange.max ? `₹${Number(firstRange.max).toLocaleString("en-IN")}` : "";
    return min && max ? `${min} - ${max}` : min || max;
  };

  const statusConfig = {
    Pending: { bg: "bg-yellow-100", text: "text-yellow-700", border: "border-yellow-300" },
    Reviewing: { bg: "bg-blue-100", text: "text-blue-700", border: "border-blue-300" },
    Shortlisted: { bg: "bg-purple-100", text: "text-purple-700", border: "border-purple-300" },
    Interviewed: { bg: "bg-indigo-100", text: "text-indigo-700", border: "border-indigo-300" },
    Selected: { bg: "bg-green-100", text: "text-green-700", border: "border-green-300" },
